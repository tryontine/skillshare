  ...
  return (
    <section className="page-frame py-10 sm:py-16">
      <div className="grid gap-4 lg:grid-cols-12 lg:grid-rows-[1.2fr_0.8fr]">
        <div className="mesh-panel relative overflow-hidden rounded-[36px] p-8 text-white lg:col-span-7 lg:row-span-2 lg:p-10">
          <p className="text-xs font-semibold uppercase tracking-[0.32em] text-white/70">
            Switzerland-only marketplace
          </p>
          <h1 className="mt-6 max-w-2xl font-display text-6xl leading-none sm:text-7xl">
        ...
        <div className="rounded-[36px] border border-[--color-line] bg-white/84 p-6 lg:col-span-5">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[--color-copper]">
            Featured this week
          </p>
          <div className="mt-4 space-y-4">
            {featured.slice(0, 2).map((service) => (
              <div key={service.id} className="rounded-[24px] bg-[--color-surface] p-4">
                <p className="text-xs uppercase tracking-[0.18em] text-[--color-ink-soft]">
                  {service.city}
                </p>
                <h3 className="mt-2 font-display text-3xl">{service.title}</h3>
                <p className="mt-2 text-sm text-[--color-ink-soft]">{service.summary}</p>
              </div>
            ))}
          </div>
        </div>
        <Link
          ...
          className="group rounded-[36px] border border-[--color-line] bg-[--color-surface] p-6 lg:col-span-3"
        >
        ...
        <div className="rounded-[36px] border border-[--color-line] bg-white/84 p-6 lg:col-span-2">
          <p className="text-xs uppercase tracking-[0.22em] text-[--color-copper]">
            Cities live
          </p>
